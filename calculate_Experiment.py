...
#         np.save(ex_stress[k]+'_beta_sum', aband["beta_sum{0}".format(k)])
    ...
    hpwr = {}
    for i in range(0,x):
         hpwr["happy{0}".format(i)]=Feature_calc.abs_psd_feature(h["norm{0}".format(i)], Fs)
         np.save(ex_norm[i]+'_abspwr', hpwr["happy{0}".format(i)])
    spwr ={}
    for j in range(0,y):
         spwr["sad{0}".format(j)]=Feature_calc.abs_psd_feature(s["calm{0}".format(j)], Fs)
         np.save(ex_calm[j]+'_abspwr', spwr["sad{0}".format(j)])
         
    apwr = {}     
    for k in range(0,z):
        apwr["angry{0}".format(k)] = Feature_calc.abs_psd_feature(a["stress{0}".format(k)],Fs)
    ...
    for i in range(0,x):
         hpsd["hspec{0}".format(i)] = Feature_calc.spec(h["norm{0}".format(i)], Fs)
         np.save(ex_norm[i]+'_spec', hpsd["hspec{0}".format(i)])
    for j in range(0,y):
         spsd["sspec{0}".format(j)] = Feature_calc.spec(s["calm{0}".format(j)], Fs)
         np.save(ex_calm[j]+'_spec', spsd["sspec{0}".format(j)])
    for k in range(0,z):
        apsd["aspec{0}".format(k)] = Feature_calc.spec(a["stress{0}".format(k)], Fs)
